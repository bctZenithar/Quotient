...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
	    ...
	    //printf("CWallet::CreateCoinStake: block: %s tx: %s", block.GetHash().ToString().c_str(), pcoin.first->GetHash().ToString().c_str());
            if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake, targetProofOfStake))
            {
				...
				{
					uint64_t nTotalSize = pcoin.first->vout[pcoin.second].nValue + GetProofOfStakeReward(nCoinAge, 0, pindexBest->nHeight);
					if (nTotalSize / 2 > nStakeSplitThreshold * COIN || nCredit > nPreferredBlockSize)
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
            ...
            && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if ( nCredit >= nPreferredBlockSize)
                break;
            // Stop adding inputs if reached reserve limit
            if (nCredit + pcoin.first->vout[pcoin.second].nValue > nBalance - nReserveBalance)
                break;
            // Do not add additional significant input
            if (pcoin.first->vout[pcoin.second].nValue >= nPreferredBlockSize)
                continue;
...

    {
	     ...
	     BOOST_FOREACH(const COutput& out, vCoins)
		{
			...
			if (out.tx->IsCoinStake() && out.tx->GetBlocksToMaturity() == 0  && out.tx->GetDepthInMainChain() == nCoinbaseMaturity+10)
			{
				...
				int nChangePos;
				bool fCreated = CreateTransaction(vecSend, wtx, keyChange, nFeeRet, nChangePos, 1, cControl);
				if (!fCreated)
